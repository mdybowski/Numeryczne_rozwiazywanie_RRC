function u = du_fort_frankel_Dirichlet(x_p, x_k, t_k, b, mu,...
    f, u_0, h,rysuj,t_0)
% Parametry
k = mu*h*h;
...
% Schemat Du Forta-Frankela
for n = 3:n_init
    u(2:end-1, n) = (2*b*mu/(1+2*b*mu))*(u(3:end, n-1) ...
        + u(1:end-2, n-1)) + ((1-2*b*mu)/(1+2*b*mu))*u(2:end-1, n-2)...
        + 2*k*f(2:end-1, n);   
end
